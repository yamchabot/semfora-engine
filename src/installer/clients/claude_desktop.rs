...
impl McpClient for ClaudeDesktopClient {
    ...
    fn detect(&self, platform: &Platform) -> ClientStatus {
        ...
        if let Some(config_path) = &paths.claude_desktop {
    ...
    fn config_path(&self, platform: &Platform) -> Option<PathBuf> {
        McpClientPaths::for_platform(platform).claude_desktop
    }

    fn project_config_path(&self) -> Option<PathBuf> {
        // Claude Desktop doesn't support project-level configuration
        None
    }
